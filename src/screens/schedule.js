...
import {View, Text, StatusBar, StyleSheet, Image, FlatList, ScrollView, Switch} from "react-native"
            ...
            <View style={scheduleScreenStyle.schedule}>
                ...
                <ScrollView style={scheduleStyle.container}>
                {
                    scheduleList.map((scheduleItem, idx) => {
                        ...
                        return (
                            <View 
                                style={scheduleItemStyle.container}
                                key={idx}
                            >
                                <View style={scheduleItemStyle.equipmentInfo}
                                >
                                    <Text style={scheduleItemStyle.timeText}>{hour}:{minute}</Text>
                                    <Text style={scheduleItemStyle.equipmentText}>Light {scheduleItem.isEnabled ? "ON" : "OFF"}, Fan {scheduleItem.fanIdx}</Text>
                                </View>
                                
                                <ToggleSwitch
                                   value={scheduleItem.isEnabled}
                                   onValueChange={() => handleScheduleItemToggleSwitch(idx)}
                                   activeText={"On"}
                                   inActiveText={"Off"}
                                   backgroundInactive={"#FCF5E5"}
                                   backgroundActive={"#90EE90"}
                                   inactiveTextStyle={scheduleItemToggleSwitchStyle.inactiveText}
                                   activeTextStyle={scheduleItemToggleSwitchStyle.activeText}
                                   switchHeight={30}
                                   switchWidth={30}
                                   containerStyle={scheduleItemToggleSwitchStyle.container}
                                />
                            </View>
                        )
                    })
